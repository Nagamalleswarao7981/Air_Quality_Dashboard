...
void setupWeb() {
  ...
  server.on("/api/network/status", HTTP_GET, []() {
    String json = "{";
    json += "\"status\":\"" + network.getStatusString() + "\",";
    json += "\"color\":\"" + network.getStatusColor() + "\",";
    json += "\"state\":" + String(network.getState()) + ",";

    NetworkInfo net = network.getNetworkInfo();
    json += "\"operator\":\"" + net.operatorName + "\",";
    json += "\"ip\":\"" + net.ip + "\",";
    json += "\"tech\":\"" + net.accessTechnology + "\",";
    json += "\"rssi\":" + String(network.getSignalStrength().rssi) + ",";
    json += "\"dbm\":" + String(network.getSignalStrength().dbm) + ",";
    json += "\"mqttConnected\":" +
            String(network.isMqttConnected() ? "true" : "false");
    json += "}";
    server.send(200, "application/json", json);
  });
