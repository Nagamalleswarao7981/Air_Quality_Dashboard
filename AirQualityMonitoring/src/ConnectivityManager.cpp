...
bool ConnectivityManager::configureMQTT() {
  ...
  sendATWait("AT+QMTCLOSE=0");

  // Config
  sendATWait("AT+QMTCFG=\"version\",0,4"); // MQTT 3.1.1
  sendATWait("AT+QMTCFG=\"keepalive\",0,60");
  sendATWait("AT+QMTCFG=\"recv/mode\",0,0"); // Passive mode

  // Open
  String openCmd = "AT+QMTOPEN=0,\"" + _broker + "\"," + String(_port);
  String resp = sendATWait(openCmd, 3000);

  // Wait for +QMTOPEN: 0,0 (Success)
  if (resp.indexOf("+QMTOPEN: 0,0") != -1)
    return true;
  if (resp.indexOf("+QMTOPEN: 0,2") != -1)
    return true;
...
bool ConnectivityManager::publish(String topic, String payload) {
  ...
  // We use msgID 0 (or auto-increment if needed), QoS 0, Retain 0
  String cmd = "AT+QMTPUB=0,0,0,0,\"" + topic + "\",\"" + payload + "\"";
  String resp = sendATWait(cmd, 2000);

  // Expect +QMTPUB: 0,0,0 (Success)
